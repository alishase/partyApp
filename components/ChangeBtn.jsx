...
export default function ChangeBtn({ card, person}) {
    ...
    async function updateCard(e) {
      ...
      if(session?.user.email=='maksimzorin10@gmail.com'){
        return (
            <button onClick={(e)=> {updateCard(e)}} className="toggle-payment-status  bg-blue-500 duration-300 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded text-xs ml-2">
                Изменить
            </button>
        )
    }
